...
def funcs(NUMBERINFECTED,time_length, YINF,MUIND,phiB,phiO):
    for ind in range(NUMBERINFECTED):
        ...
        for t in range(time_length-1):
            ...
            # 0 (P) is killed by 4 (I) and proliferate 
            YINF[ind][0][t+1] = YINF[ind][0][t] + h*( (-phiB*YINF[ind][0][t] + -MUIND[ind][0][4] * YINF[ind][4][t] * (MUIND[ind][0][4] < 0 and (1.0 - YINF[ind][0][t]) or YINF[ind][0][t])) + (PROLIF * YINF[ind][0][t] * (1 - YINF[ind][0][t]) )  )
            
            
            # 0 (P) activates 1 (R1)
            YINF[ind][1][t+1] = YINF[ind][1][t] + h*( -0*YINF[ind][1][t] +  1 * YINF[ind][0][t] * (1.0 - YINF[ind][1][t]) ) 
            
            
            # 1 (R1) activates 2 (R2) and is reduced by 5 (U)
            YINF[ind][2][t+1] = YINF[ind][2][t] + h*( (-0*YINF[ind][2][t] +  MUIND[ind][2][1] * YINF[ind][1][t] * (MUIND[ind][2][1] > 0 and (1.0 - YINF[ind][2][t]) or YINF[ind][2][t]) ) +(MUIND[ind][2][5] * YINF[ind][5][t] * (MUIND[ind][2][5] > 0 and (1.0 - YINF[ind][2][t]) or YINF[ind][2][t]) ))
            
            
            # 3 (A) is activated by 2 (R2)
            YINF[ind][3][t+1] = YINF[ind][3][t] + h*(  -phiO*YINF[ind][3][t] +  MUIND[ind][3][2] * YINF[ind][2][t] * (MUIND[ind][3][2] > 0 and (1.0 - YINF[ind][3][t]) or YINF[ind][3][t]))
            
            # 4 (I) is activated by 3 (A) and deactivated by D (6)
            YINF[ind][4][t+1] = YINF[ind][4][t] + h*( (-phiO*YINF[ind][4][t] +  MUIND[ind][4][3] * YINF[ind][3][t] * (MUIND[ind][4][3] > 0 and (1.0 - YINF[ind][4][t]) or YINF[ind][4][t]) ) +(MUIND[ind][4][6] * YINF[ind][6][t] * (MUIND[ind][4][6] > 0 and (1.0 - YINF[ind][4][t]) or YINF[ind][4][t])) )
            
            #5 (U) is activated by 3 (A)
            YINF[ind][5][t+1] = YINF[ind][5][t] + h*(  -phiO*YINF[ind][5][t] +  MUIND[ind][5][3] * YINF[ind][3][t] * (MUIND[ind][5][3] > 0 and (1.0 - YINF[ind][5][t]) or YINF[ind][5][t]))
            
            
            #6 (D) is activated by 3 (A)
            YINF[ind][6][t+1] = YINF[ind][6][t] + h*(  -phiO*YINF[ind][6][t] +  MUIND[ind][6][3] * YINF[ind][3][t] * (MUIND[ind][6][3] > 0 and (1.0 - YINF[ind][6][t]) or YINF[ind][6][t]))
...
cmbefore = [ 
    ...
    [0,  0,  0,  0,  0,  0], #y0 [Receptor1]
    
    [1,  0,  0,  0,  1,  0], #y1 [Receptor2]
...
cm = [
    ...
    [0,  0,  0,  0,  1,  0,  0], #y0 [Pathogen]
    
    [1,  0,  0,  0,  0,  0,  0], #y1 [Receptor1]
    
    [0,  1,  0,  0,  0,  1,  0], #y2 [Receptor2]
    
    [0,  0,  1,  0,  0,  0,  0], #y3 [Activator]
    
    [0,  0,  0,  1,  0,  0,  1], #y4 [Immunity]
    
    [0,  0,  0,  1,  0,  0,  0], #y5 [Up NFL]
    
    [0,  0,  0,  1,  0,  0,  0], #y6 [Down NFL]
]
...
#Initialize parasites with random proteins 
...
for _ in range(NUMBERGENR):
    ...
    for ind in range(len(SYSTEMSHP)):
        FITNESSINFH[ind] = np.exp(-(BEFFECT*np.average(YINF[ind][0]) + IEFFECT*np.average(YINF[ind][4])) )
